...
def upgrade() -> None:
    ...
    op.create_table('HappyHourLocations',
    ...
    op.create_table('HappyHourEvents',
    sa.Column('id', sa.Integer(), nullable=False),
    ...
    sa.ForeignKeyConstraint(['LocationID'], ['HappyHourLocations.id'], ),
    sa.ForeignKeyConstraint(['TyrantID'], ['accounts.id'], ),
    sa.PrimaryKeyConstraint('id'),
    sa.UniqueConstraint('week_of', name='uq_events_week_of'),
    )
    with op.batch_alter_table('HappyHourEvents', schema=None) as batch_op:
        batch_op.create_index(batch_op.f('ix_HappyHourEvents_When'), ['When'], unique=False)

    op.create_table('HappyHourTyrantRotation',
    sa.Column('id', sa.Integer(), nullable=False),
    ...
    op.create_table('account_claim_requests',
    sa.Column('id', sa.Integer(), nullable=False),
    sa.Column('requester_provider', sa.Integer(), nullable=False, comment='OIDC provider of the user making the claim'),
    sa.Column('requester_external_id', sa.String(), nullable=False, comment="OIDC 'sub' of the user making the claim"),
    sa.Column('requester_name', sa.String(), nullable=False, comment='Display name from the OIDC identity (for admin review)'),
    sa.Column('requester_email', sa.String(), nullable=True, comment='Email from the OIDC identity (for admin review)'),
    sa.Column('target_account_id', sa.Integer(), nullable=False, comment='The legacy account being claimed'),
    sa.Column('status', sa.String(), nullable=False),
    ...
    sa.CheckConstraint('"PayerId" != "RecipientId"', name='ck_receipts_no_self_payment'),
    sa.CheckConstraint('"Credits" > 0', name='ck_receipts_positive_credits'),
    sa.ForeignKeyConstraint(['PayerId'], ['accounts.id'], ),
    sa.ForeignKeyConstraint(['RecipientId'], ['accounts.id'], ),
    sa.ForeignKeyConstraint(['RecorderId'], ['accounts.id'], ),
    sa.PrimaryKeyConstraint('id')
    )
